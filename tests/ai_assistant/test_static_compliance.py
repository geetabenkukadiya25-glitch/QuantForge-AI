"""Static confirmation of the AI Research Assistant's hard boundaries:
no source file can place a broker order, connect to MT5, trade, optimize,
validate, replay, rebuild a strategy, or call an external AI/network
service. This mirrors the same grep-style static scan every prior engine
(`ai_extraction`, `portfolio_engine`, `research_engine`, `knowledge_base`)
...
MODULE_DIR = Path(__file__).resolve().parents[2] / "app" / "ai_assistant"
...
def test_never_imports_upstream_engine_execution_logic() -> None:
    ...
    another engine's orchestrator, runner, simulator, controller, or
    builder."""
    offenders = []
    for path in MODULE_DIR.glob("*.py"):
...
def test_no_embeddings_or_vector_database_dependency() -> None:
    """Defense-in-depth: this module must never import a vector-search/embedding library."""
    forbidden_libs = ("faiss", "chromadb", "pinecone", "weaviate", "sentence_transformers", "import openai_embeddings")
    offenders = []
    for path in MODULE_DIR.glob("*.py"):
        text = path.read_text(encoding="utf-8").lower()
