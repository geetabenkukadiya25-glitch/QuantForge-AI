"""Confirms adding the Research Engine didn't touch or break any prior phase."""
...
def test_paths_still_exposes_every_prior_directory() -> None:
    ...
    for field in (
        ...
        "research_engine_dir", "research_results_dir",
    ):
        assert getattr(paths, field) is not None


def test_research_engine_dir_points_inside_the_app_tree() -> None:
    paths = get_paths()
    assert paths.research_engine_dir == paths.app / "research_engine"
    assert paths.research_results_dir.exists()
