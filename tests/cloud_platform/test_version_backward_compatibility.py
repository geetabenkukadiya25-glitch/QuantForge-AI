"""Confirms adding the Project Versioning & Snapshot System (Phase 17.3)
didn't touch or break the Cloud Platform Foundation, Workspace
Management, Local Artifact Registry, or any prior engine."""
...
def test_workspace_management_still_works_standalone() -> None:
    manager = CloudWorkspaceManager()
    record = manager.create_workspace("ws1", label="Alpha")
    assert manager.registry.load("ws1") == record


def test_artifact_registry_still_works_standalone() -> None:
    manager = CloudArtifactManager()
    from app.cloud_platform.artifact import ArtifactType

    record = manager.create_artifact("a1", ArtifactType.DATASET, "Dataset A")
...
def test_paths_still_exposes_every_prior_directory() -> None:
    ...
    for field in (
        ...
        "optimization_engine_dir", "validation_engine_dir", "replay_engine_dir", "research_engine_dir",
        "knowledge_base_dir", "ai_extraction_dir", "portfolio_engine_dir", "ai_assistant_dir", "ea_generator_dir",
    ):
        assert getattr(paths, field) is not None


def test_versioning_adds_no_new_path_fields() -> None:
    """This phase is in-memory only -- no new results directory on disk."""
    paths = get_paths()
    assert not hasattr(paths, "version_registry_dir")
    assert not hasattr(paths, "cloud_platform_dir")


def test_version_modules_import_cleanly_alongside_everything_else() -> None:
    import app.cloud_platform  # noqa: F401
    import app.cloud_platform.artifact_manager  # noqa: F401
    import app.cloud_platform.version_manager  # noqa: F401
    import app.cloud_platform.version_registry  # noqa: F401
    import app.cloud_platform.version_report  # noqa: F401
    import app.cloud_platform.version_statistics  # noqa: F401
    import app.cloud_platform.versioning  # noqa: F401
