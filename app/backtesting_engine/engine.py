...
class BacktestingEngine(BaseEngine):
    ...
    def execute(self, strategy_model: StrategyModel, data: pd.DataFrame, configuration: BacktestConfiguration) -> BacktestResult:
        """Run one full backtest, raising on validation failure.

        Raises:
            BacktestValidationError: if the context fails pre-execution validation.
        """
        return self._runner.execute(self._build_context(strategy_model, data, configuration))

    def try_execute(self, strategy_model: StrategyModel, data: pd.DataFrame, configuration: BacktestConfiguration) -> BacktestSession:
        """Run one full backtest. Never raises -- inspect the returned session."""
        return self._runner.try_execute(self._build_context(strategy_model, data, configuration))
