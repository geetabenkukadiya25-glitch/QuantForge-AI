"""Equal Lows detector: clusters of swing lows within a price tolerance."""
...
class EqualLowDetector(BaseSMCDetector):
    """Two or more swing lows whose prices sit within a small tolerance of each other."""
    ...
    def metadata(cls) -> SMCMetadata:
        return SMCMetadata(
            name="Equal Low",
            category="Liquidity",
            description="Clusters of swing lows within a small price tolerance of each other.",
            inputs=("Low",),
            outputs=("equal_low",),
            parameters=(
    ...
    def _detect(self, context: SMCContext) -> list[SMCDetection]:
        ...
        data = context.data
        swing_indices = find_swing_lows(data, left, right)
        prices = {i: float(data["Low"].iloc[i]) for i in swing_indices}
    ...
    def _build(self, context: SMCContext, prices: dict[int, float], cluster: list[int]) -> SMCDetection:
        ...
        return SMCDetection(
            ...
            label="Equal Lows",
            price=avg_price,
            notes=f"{len(cluster)} swing low(s) clustered near {avg_price:.5f}.",
        )
