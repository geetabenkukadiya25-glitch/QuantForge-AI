"""Registers, searches, enables, and disables completed `OptimizationResult`s.

An in-memory registry (mirroring `BacktestRegistry`'s shape), keyed by
`result_id` since a single strategy can have many optimization runs.
...
FLAG_PREFIX = "optimization."


class OptimizationRegistry:
    """Tracks known completed `OptimizationResult`s and their enabled/disabled state."""
    ...
    def register(self, result: OptimizationResult, overwrite: bool = False) -> None:
        """Register `result`, making it available and enabled by default.

        Raises:
            OptimizationRegistrationError: if a result is already
                registered under this result id and `overwrite` is False.
        """
        result_id = result.result_id
        if result_id in self._results and not overwrite:
            raise OptimizationRegistrationError(
                f"Optimization result {result_id!r} is already registered. Pass overwrite=True to replace it."
            )
    ...
    def load(self, result_id: str) -> OptimizationResult:
        """Return the registered result for `result_id`.

        Raises:
            OptimizationNotFoundError: if `result_id` isn't registered.
        """
        try:
            return self._results[result_id]
        except KeyError as exc:
            raise OptimizationNotFoundError(f"Unknown optimization result: {result_id!r}") from exc

    def is_registered(self, result_id: str) -> bool:
        return result_id in self._results

    def require_enabled(self, result_id: str) -> OptimizationResult:
        """Load `result_id`, refusing if it's currently disabled.

        Raises:
            OptimizationNotFoundError: if `result_id` isn't registered.
            OptimizationDisabledError: if `result_id` is registered but disabled.
        """
        if not self.is_enabled(result_id):
            raise OptimizationDisabledError(f"Optimization result {result_id!r} is disabled.")
    ...
    def list(self, include_disabled: bool = True) -> list[OptimizationMetadata]:
    ...
    def search(self, strategy_id: str | None = None) -> list[OptimizationMetadata]:
