"""Registers, searches, enables, and disables completed `ValidationResult`s.

An in-memory registry (mirroring `OptimizationRegistry`'s shape), keyed
by `result_id` since a single strategy can have many validation runs.
...
FLAG_PREFIX = "validation."


class ValidationRegistry:
    """Tracks known completed `ValidationResult`s and their enabled/disabled state."""
    ...
    def register(self, result: ValidationResult, overwrite: bool = False) -> None:
        """Register `result`, making it available and enabled by default.

        Raises:
            ValidationRegistrationError: if a result is already registered
                under this result id and `overwrite` is False.
        """
        result_id = result.result_id
        if result_id in self._results and not overwrite:
            raise ValidationRegistrationError(
                f"Validation result {result_id!r} is already registered. Pass overwrite=True to replace it."
            )
    ...
    def load(self, result_id: str) -> ValidationResult:
        """Return the registered result for `result_id`.

        Raises:
            ValidationNotFoundError: if `result_id` isn't registered.
        """
        try:
            return self._results[result_id]
        except KeyError as exc:
            raise ValidationNotFoundError(f"Unknown validation result: {result_id!r}") from exc

    def is_registered(self, result_id: str) -> bool:
        return result_id in self._results

    def require_enabled(self, result_id: str) -> ValidationResult:
        """Load `result_id`, refusing if it's currently disabled.

        Raises:
            ValidationNotFoundError: if `result_id` isn't registered.
            ValidationDisabledError: if `result_id` is registered but disabled.
        """
        if not self.is_enabled(result_id):
            raise ValidationDisabledError(f"Validation result {result_id!r} is disabled.")
    ...
    def list(self, include_disabled: bool = True) -> list[ValidationMetadata]:
    ...
    def search(self, strategy_id: str | None = None) -> list[ValidationMetadata]:
