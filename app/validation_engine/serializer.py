"""Serializes `ValidationResult`s to dict/JSON/YAML."""
...
class ValidationSerializer:
    """Converts `ValidationResult` instances to plain dict/JSON/YAML."""

    def to_dict(self, result: ValidationResult) -> dict:
        """Return `result` as a plain, JSON-safe dict."""
        return result.model_dump(mode="json")

    def to_json(self, result: ValidationResult, pretty: bool = True, canonical: bool = False) -> str:
        data = self.to_dict(result)
        if canonical:
            return json.dumps(data, sort_keys=True, separators=(",", ":"))
        return json.dumps(data, indent=2 if pretty else None, sort_keys=False)

    def to_yaml(self, result: ValidationResult, canonical: bool = False) -> str:
        data = self.to_dict(result)
