...
uploaded_file = st.sidebar.file_uploader("CSV file (standard or MT5 export format)", type=["csv"])
...
chart_type = st.sidebar.radio("Chart type", ["candlestick", "ohlc"], horizontal=True)
theme_name = st.sidebar.selectbox("Theme", ["dark", "light"])
timeframe = st.sidebar.selectbox("Timeframe", TIMEFRAMES, index=TIMEFRAMES.index("H1"))
show_volume = st.sidebar.checkbox("Show volume", value=True)
show_sessions = st.sidebar.checkbox("Show market sessions", value=False)
fullscreen = st.sidebar.checkbox("Fullscreen height", value=False)
...
drawing_type = st.sidebar.selectbox(
    "Tool",
    [
        "Horizontal Line",
        "Vertical Line",
...
with st.sidebar.form("add_drawing_form"):
...
if st.sidebar.button("Clear drawings"):
    st.session_state.drawing_manager.clear()

st.sidebar.caption(f"{len(st.session_state.drawing_manager.list())} drawing(s) on chart")
