"""Registers, searches, enables, and disables completed `EAGeneratorResult`s.

An in-memory registry (mirroring `PortfolioRegistry`'s shape), keyed by
`result_id` since a single strategy can back many generated EAs (e.g.
different output filenames or risk configurations). Enable/disable is
...
FLAG_PREFIX = "ea_generator."


class EAGeneratorRegistry:
    """Tracks known completed `EAGeneratorResult`s and their enabled/disabled state."""

    def __init__(self, feature_flags: FeatureFlagManager | None = None) -> None:
        self._feature_flags = feature_flags or FeatureFlagManager()
        self._results: dict[str, EAGeneratorResult] = {}

    def register(self, result: EAGeneratorResult, overwrite: bool = False) -> None:
        """Register `result`, making it available and enabled by default.

        Raises:
            EARegistrationError: if a result is already registered
                under this result id and `overwrite` is False.
        """
        result_id = result.result_id
        if result_id in self._results and not overwrite:
            raise EARegistrationError(f"EA generation result {result_id!r} is already registered. Pass overwrite=True to replace it.")
    ...
    def load(self, result_id: str) -> EAGeneratorResult:
        """Return the registered result for `result_id`.

        Raises:
            EANotFoundError: if `result_id` isn't registered.
        """
        try:
            return self._results[result_id]
        except KeyError as exc:
            raise EANotFoundError(f"Unknown EA generation result: {result_id!r}") from exc

    def is_registered(self, result_id: str) -> bool:
        return result_id in self._results

    def require_enabled(self, result_id: str) -> EAGeneratorResult:
        """Load `result_id`, refusing if it's currently disabled.

        Raises:
            EANotFoundError: if `result_id` isn't registered.
            EADisabledError: if `result_id` is registered but disabled.
        """
        if not self.is_enabled(result_id):
            raise EADisabledError(f"EA generation result {result_id!r} is disabled.")
    ...
    def list(self, include_disabled: bool = True) -> list[EAGeneratorMetadata]:
    ...
    def search(self, strategy_id: str | None = None) -> list[EAGeneratorMetadata]:
        """Return metadata for registered results generated from `strategy_id`."""
        results = self.list()
        if strategy_id:
