"""Standalone OHLC (bar) chart rendering."""
...
class OHLCChart:
    """Builds an OHLC bar `go.Figure` from OHLCV data."""

    def build(self, df: pd.DataFrame, config: ChartConfig | None = None) -> go.Figure:
        """Return a standalone OHLC figure for `df`.
    ...
    def add_trace(fig: go.Figure, df: pd.DataFrame, theme, row: int | None = None, col: int | None = None) -> None:
        """Add an OHLC trace for `df` to an existing figure/subplot."""
        trace = go.Ohlc(
            x=df[DATETIME_COL],
            open=df[OHLC_COLS[0]],
            high=df[OHLC_COLS[1]],
